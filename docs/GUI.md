Phase 7: Minimal GUI Implementation
Overview
...
Timeline: 8-10 weeks
Prerequisites: Phase 6 complete (interactive shell, PS/2 keyboard, framebuffer console, multi-process scheduler)
...
Implementation:
rust// src/drivers/mouse/ps2_mouse.rs
...
rust// src/arch/amd64/interrupts/mouse.rs
...
Implementation:
rust// src/drivers/display/cursor.rs
...
Integration:
rust// src/drivers/display/mod.rs
...
sys_poll_event(event_buf: *mut Event) -> i32
Returns 1 if event available, 0 if none, -1 on error.
rustpub fn sys_poll_event(event_buf: *mut Event) -> i32 {
    let current = current_process();
    
    // Validate buffer
    if !current.address_space.is_user_address(event_buf as u64, core::mem::size_of::<Event>()) {
        return -1;
    }
    
    // Poll event queue
    match gui::poll_event() {
        Some(event) => {
            unsafe {
                *event_buf = event;
            }
...
Syscall numbers:
rustconst SYS_WINDOW_CREATE: usize = 100;
...
`userspace/librustica_gui/
src/window.rs`

userspace/librustica_gui/src/widgets.rs

Implementation:
rust// userspace/librustica_gui/src/lib.rs
...
Window wrapper:
rust// userspace/librustica_gui/src/window.rs
...
Widget abstractions:
rust// userspace/librustica_gui/src/widgets.rs
...
Implementation:
rust// userspace/desktop/src/main.rs
...
Implementation:
rust// userspace/app-launcher/src/main.rs
...
Implement feature in kernel
Add corresponding syscall
Update librustica_gui
Test with simple demo app
Integrate into desktop shell
...
rustux/README.md - Add Phase 7 status
docs/PLAN.md - Mark Phase 7 complete when done
docs/GUI.md - Create GUI architecture documentation
docs/SYSCALLS.md - Document GUI syscalls
